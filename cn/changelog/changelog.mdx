...
一个增强的推荐系统，具有每日限制和奖励宝箱，以推动可持续增长和高质量贡献。

- **运作方式：** 每天邀请新用户的上限为 10 人。用户每邀请 5 人可获得 1 个宝箱。当被邀请用户提交并通过首次任务审核时，邀请者和被邀请者各获得 1 个宝箱，内含 1–100 随机积分。推荐页面显示实时进度和宝箱状态。满足条件时宝箱会自动发放，可在"贡献与赢取"中打开。
- **原因：** 每日上限可防止垃圾邀请，保持邀请的意义。宝箱鼓励双方做出真实贡献，提升数据质量。随机积分奖励使邀请变得有趣且公平，支持长期活动和平台增长。
...
export const ChangelogFilter = () => {
  ...
  const filterTypes = [
    { id: 'all', label: '全部', color: '#6b7280', count: 25 },
    { id: 'core-feature', label: '核心功能发布', color: '#16A34A', count: 4 },
    { id: 'optimization', label: '调整与优化', color: '#F59E0B', count: 4 },
...
export const MonthFilter = () => {
  ...
  const months = [
    { id: 'all', label: '全部月份', count: 25 },
    { id: 'jan', label: '2026 年 1 月', count: 1 },
    { id: 'dec', label: '2025 年 12 月', count: 1 },
