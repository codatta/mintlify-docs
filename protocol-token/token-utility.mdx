...
`$XNY` controls access, trust, rights, and governance. Multi‑asset payouts reduce buyer friction and meet cost‑management/compliance 
constraints (including stablecoin‑only settlement when required), while keeping `$XNY` as the network’s control and fee currency.
