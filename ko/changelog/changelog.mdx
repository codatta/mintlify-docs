...
지속 가능한 성장과 고품질 기여를 촉진하기 위한 일일 제한 및 보상 상자가 있는 향상된 추천 시스템입니다.

- **방법:** 매일 신규 사용자 10명까지 초대 제한. 사용자는 5명 초대할 때마다 상자 1개를 획득합니다. 초대받은 사용자가 첫 번째 작업 검토를 제출하고 통과하면, 초대자와 피초대자 모두 각각 1개의 상자를 받으며, 1-100개의 무작위 포인트가 포함됩니다. 추천 페이지는 실시간 진행 상황과 상자 상태를 표시합니다. 조건이 충족되면 상자가 자동으로 발급되며 "기여 및 승리"에서 열 수 있습니다.
- **이유:** 일일 상한은 스팸을 방지하고 초대를 의미 있게 유지합니다. 상자는 양쪽에서 실제 기여를 장려하여 데이터 품질을 향상시킵니다. 무작위 포인트 보상은 초대를 재미있고 공정하게 만들어 장기 활동과 플랫폼 성장을 지원합니다.
...
export const ChangelogFilter = () => {
  ...
  const filterTypes = [
    { id: 'all', label: '전체', color: '#6b7280', count: 25 },
    { id: 'core-feature', label: '핵심 기능 출시', color: '#16A34A', count: 4 },
    { id: 'optimization', label: '조정 및 최적화', color: '#F59E0B', count: 4 },
...
export const MonthFilter = () => {
  ...
  const months = [
    ...
    { id: 'dec', label: '2025년 12월', count: 1 },
