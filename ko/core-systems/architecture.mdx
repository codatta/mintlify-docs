...
> **요약**  
> Codatta는 인간/에이전트의 기여를 **검증 가능하고 소유 가능한 데이터 자산**으로 전환합니다. 계보가 추적 가능하고, 정책으로 게이트/관찰되는 접근, 증명 가능한 소유권, 그리고 **재현 가능한 로열티 분배**를 제공합니다.
...
1. **기여(Contribution)**: 기여자(인간/에이전트)가 제출한 작업은 **[기여 지문](/core-concepts/contribution-fingerprint.mdx)** 으로 고정되고 **[아이덴티티](/core-concepts/identity.mdx)**, **[평판](/core-concepts/reputation.mdx)** 과 결속됩니다.
2. **자산화(Assetification)**: 검증된 지문을 갖는 원자적 기여(샘플/레이블/검증)를 모아 버전이 관리되는 데이터셋(**[계보 & 자산 조립](/core-concepts/lineage-asset-assembly.mdx)**)을 만들고, 온체인 **[토큰화 소유권](/core-concepts/tokenized-ownership-proofs.mdx)**(양도 가능한 분수)으로 **민팅**합니다.
...
4. **로열티(Royalty)**: **[로열티 엔진](/core-concepts/royalty-engine.mdx)** 이 매출, 시점 고정 소유권 분수, 계량 이벤트를 결합해 소유자와 금고에 **결정적 분배**를 수행합니다.
...
- **지식 후원자(스테이커/언더라이터/구매자)**:  
  - **스테이킹/언더라이팅**으로 **지분을 확보/상향**; 감사 결과에 따라 거버넌스 한도 내 **에서 증가하거나 감소**될 수 있습니다.  
  - **2차 시장에서 지분 매수**(유동성 활성화된 경우). 이는 **분배 수학**을 바꾸지 않고, **수령자만** 바꿉니다.  
